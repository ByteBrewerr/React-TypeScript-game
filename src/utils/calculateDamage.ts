  ...
  const oneUnitDamage =
    attackerCharacter.minDamage === attackerCharacter.maxDamage
      ...
      : parseFloat(
          (
            Math.random() *
              (attackerCharacter.maxDamage - attackerCharacter.minDamage) +
            attackerCharacter.minDamage
          ).toFixed(1),
        );
  ...
  if (isAttackMoreThenDefence) {
    totalDamage = Math.round(
      oneUnitDamage *
        ...
        ((attackerCharacter.assault - targetCharacter.defence) * 0.05 + 1),
    );
  } else {
    totalDamage = Math.round(
      (oneUnitDamage * attackerCharacter.count) /
        ((targetCharacter.defence - attackerCharacter.assault) * 0.05 + 1),
    );
  }
  return {
    totalDamage,
